...
terraform {
  backend "s3" {
    ...
    key     = "sharemyhealth/terraform.tfstate"
    encrypt = true
  }
}
...
# sec group
resource "aws_security_group" "sharemyhealth" {
  vpc_id      = "${data.aws_vpc.default.id}"
  name        = "sharemyhealth-${var.environment}"
...
resource "aws_security_group_rule" "allow-postgres" {
  ...
  source_security_group_id = "${aws_security_group.sharemyhealth.id}"
}
...
resource "aws_ecr_repository" "sharemyhealth" {
  name = "sharemyhealth"
}

resource "aws_s3_bucket" "default" {
  bucket = "sharemyhealth.${var.environment}.bucket"
  acl    = "private"
...
data "template_file" "Dockerrun" {
  ...
  vars {
    ecr_url = "${aws_ecr_repository.sharemyhealth.repository_url}"
    version = "${var.version}"
  }
}
...
resource "aws_iam_role" "ec2" {
  name               = "sharemyhealth-eb-ec2"
  assume_role_policy = "${data.aws_iam_policy_document.ec2.json}"
}

resource "aws_iam_role_policy" "default" {
  name   = "sharemyhealth-eb-default"
...
resource "aws_elastic_beanstalk_application" "default" {
  name        = "sharemyhealth-${var.environment}"
  description = "sharemyhealth-${var.environment}-desc"
}

resource "aws_elastic_beanstalk_application_version" "default" {
  name        = "sharemyhealth-${var.environment}-${var.version}"
...
resource "aws_elastic_beanstalk_environment" "default" {
  name                = "sharemyhealth-${var.environment}-env"
  application         = "${aws_elastic_beanstalk_application.default.name}"
  ...
  setting {
    namespace = "aws:autoscaling:launchconfiguration"
    name      = "SecurityGroups"
    value     = "${aws_security_group.sharemyhealth.id}"
  }
}

resource "aws_ssm_parameter" "allowed_host" {
  name        = "/${var.environment}/sharemyhealth/eb/ALLOWED_HOSTS"
  description = "Server hostname"
  type        = "SecureString"
  value       = "alpha.sharemy.health"
